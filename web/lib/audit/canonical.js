...
function normalize(value) {
    ...
    if (typeof value === "number") {
        ...
        if (!Number.isFinite(value))
            throw new TypeError("non-finite float cannot be canonicalized");
...
function contentId(value, digestSize = 16) {
    ...
    const hex = (0, blakejs_1.blake2bHex)(Buffer.from(jsonStr, "utf8"), undefined, digestSize);
    return `blake2b${digestSize * 8}:${hex}`;
}
function deriveSeed(runSeed, ...parts) {
    const ctx = (0, blakejs_1.blake2bInit)(8);
    (0, blakejs_1.blake2bUpdate)(ctx, Buffer.from(String(runSeed), "utf8"));
    for (const part of parts) {
        (0, blakejs_1.blake2bUpdate)(ctx, Buffer.from("\u001f", "utf8"));
        (0, blakejs_1.blake2bUpdate)(ctx, Buffer.from(String(part), "utf8"));
    }
    const digest = (0, blakejs_1.blake2bFinal)(ctx);
