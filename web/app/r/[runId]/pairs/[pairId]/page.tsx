...
type Props = { params: Promise<{ runId: string; pairId: string }> };

export async function generateStaticParams() {
  const index = await loadRunIndex();
  const all = await Promise.all(index.runs.map(async (run) => (await getPairs(run.run_id)).map((pair) => ({
    runId: run.run_id,
    pairId: field(pair, "pair_id"),
  }))));
  return all.flat();
}
